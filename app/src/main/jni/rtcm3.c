...
///* decode type 0063: bds ephemerides -----------------------------------------*/
static int decode_type0063(rtcm_t *rtcm)
{
    ...
    if (i+499<=rtcm->len*8) {
        ...
        eph.sva   =getbitu(rtcm->buff,i, 4);              i+= 4;//urai
        ...
        toc       =getbitu(rtcm->buff,i,17)*8.0;         i+=17;//*16
        eph.f2    =getbits(rtcm->buff,i, 11)*pow(2.0,-66);        i+= 11;
        eph.f1    =getbits(rtcm->buff,i,22)*pow(2.0,-50);        i+=22;
        eph.f0    =getbits(rtcm->buff,i,24)*pow(2.0,-33);        i+=24;
        eph.iodc  =getbitu(rtcm->buff,i,5);              i+=5;//aodc
        eph.crs   =getbits(rtcm->buff,i,18)*pow(2.0,-6);         i+=18;
        eph.deln  =getbits(rtcm->buff,i,16)*P2_43*SC2RAD; i+=16;
        eph.M0    =getbits(rtcm->buff,i,32)*P2_31*SC2RAD; i+=32;
        eph.cuc   =getbits(rtcm->buff,i,18)*P2_31;        i+=18;//-29
        eph.e     =getbitu(rtcm->buff,i,32)*P2_33;        i+=32;
        eph.cus   =getbits(rtcm->buff,i,18)*P2_31;        i+=18;//-29
        sqrtA     =getbitu(rtcm->buff,i,32)*P2_19;        i+=32;
        eph.toes  =getbitu(rtcm->buff,i,17)*8.0;         i+=17;//*16
        eph.cic   =getbits(rtcm->buff,i,18)*P2_31;        i+=18;//-29
        eph.OMG0  =getbits(rtcm->buff,i,32)*P2_31*SC2RAD; i+=32;
        eph.cis   =getbits(rtcm->buff,i,18)*P2_31;        i+=18;//-29
        eph.i0    =getbits(rtcm->buff,i,32)*P2_31*SC2RAD; i+=32;
        eph.crc   =getbits(rtcm->buff,i,18)*pow(2.0,-6);         i+=18;//-5
        eph.omg   =getbits(rtcm->buff,i,32)*P2_31*SC2RAD; i+=32;
        eph.OMGd  =getbits(rtcm->buff,i,24)*P2_43*SC2RAD; i+=24;
        eph.tgd[0]=getbits(rtcm->buff,i, 10)*pow(10.0,-10);        i+= 10;//-31
        eph.tgd[1]=getbits(rtcm->buff,i, 10)*pow(10.0,-10);        i+= 10;//-31
        eph.svh   =getbitu(rtcm->buff,i, 1);              i+= 1;
        //eph.flag  =getbitu(rtcm->buff,i, 1);              i+= 1;
        //eph.fit   =getbitu(rtcm->buff,i, 1)?0.0:4.0; /* 0:4hr,1:>4hr */
    }
    else {
        trace(2,"rtcm3 0063 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (prn>=40) {
        sys=SYS_SBS; prn+=80;
    }
    trace(4,"decode_type0063: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        trace(2,"rtcm3 0063 satellite number error: prn=%d\n",prn);
        return -1;
    }
    //adjweek_bds(rtcm,eph.toes);
    ...
    if (!strstr(rtcm->opt,"-EPHALL")) {
        //if (eph.iode==rtcm->nav.eph[sat-1].iode) return 0; /* unchanged *///北斗数据龄期有问题，iode都是1，除9，,11,12,14
        if (eph.toes==rtcm->nav.eph[sat-1].toes) return 0;
    }
    if(sat>93)
    {
        sat=sat;
...
void adjweek_bds(rtcm_t *rtcm, double tow)
{
    double tow_p;
    int week;
...
int decode_head1104(rtcm_t *rtcm, int *sync)
{
    double tow;
    char *msg;
...
/* decode type 1104: BDS observables ----------------------*/
int decode_type1104(rtcm_t *rtcm)
{
...
//        cnr3 =getbitu(rtcm->buff,i, 8); i+= 8;
        ...
        if (ppr1!=(int)0xFFF80000) {
            rtcm->obs.data[index].P[0]=pr1;
            cp1=adjcp(rtcm,sat,0,ppr1*0.0005/(CLIGHT/FREQ1_CMP));
            rtcm->obs.data[index].L[0]=pr1/(CLIGHT/FREQ1_CMP)+cp1;
        }
        rtcm->obs.data[index].LLI[0]=lossoflock(rtcm,sat,0,lock1);
        rtcm->obs.data[index].SNR[0]=snratio(cnr1*0.25);
        ...
        if (pr2!=(int)0xFFFFE000) {
            ...
            cp2=adjcp(rtcm,sat,0,ppr2*0.0005/(CLIGHT/FREQ2_CMP));
            rtcm->obs.data[index].L[1]=pr2/(CLIGHT/FREQ2_CMP)+cp2;
        }
        rtcm->obs.data[index].LLI[1]=lossoflock(rtcm,sat,1,lock2);
        rtcm->obs.data[index].SNR[1]=snratio(cnr2*0.25);
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    switch (type) {
        ...
        case 1104: ret=decode_type1104(rtcm); break;
